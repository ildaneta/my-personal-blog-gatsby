...
const applyFilters = (filter, firstOnly, nodeTypeNames, typedKeyValueIndexes, resolvedFields) => {
  let result;
...
const runSiftOnNodes = (nodes, args, getNode = siftGetNode) => {
  ...
  return _runSiftOnNodes(nodes, filter, firstOnly, nodeTypeNames, resolvedFields, getNode);
};
...
const _runSiftOnNodes = (nodes, filter, firstOnly, nodeTypeNames, resolvedFields, getNode) => {
  let siftFilter = getFilters(liftResolvedFields(toDottedFields(prepareQueryArgs(filter)), resolvedFields)); // If the the query for single node only has a filter for an "id"
