...
var StateNode = /** @class */ (function () {
    ...
    StateNode.prototype.getDelayedTransitions = function () {
        ...
        var delayedTransitions = utils_1.isArray(afterConfig)
            ? afterConfig
            : utils_1.flatten(utils_1.keys(afterConfig).map(function (delay) {
                var configTransition = afterConfig[delay];
                var resolvedTransition = utils_1.isString(configTransition)
                    ? { target: configTransition }
                    : configTransition;
                return utils_1.toArray(resolvedTransition).map(function (transition) { return (__assign(__assign({}, transition), { delay: !isNaN(+delay) ? +delay : delay })); });
            }));
        return delayedTransitions.map(function (delayedTransition, i) {
            ...
            _this.onEntry.push(actions_1.send(eventType, { delay: delayRef }));
            _this.onExit.push(actions_1.cancel(eventType));
            return __assign(__assign({}, _this.formatTransition(__assign(__assign({}, delayedTransition), { event: eventType }))), { delay: delay });
        });
    };
    /**
